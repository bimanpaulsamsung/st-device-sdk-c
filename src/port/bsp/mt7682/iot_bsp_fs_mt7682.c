...
static xSemaphoreHandle nv_mutex = NULL;
...
static void nv_data_preload(void)
{
	...
	device_mutex_lock();
	hal_flash_read(FLASH_USR_STORAGE_BASE, (uint8_t *)&header, sizeof(nv_storage_header_s));
	...
	hal_flash_read(address, (uint8_t *)buf, pos);
...
static long nv_storage_read(const char *store, uint8_t *buf, size_t size)
{
	...
	device_mutex_lock();
	ret = hal_flash_read(nv_table[idx].addr, buf, nv_table[idx].size);
	device_mutex_unlock();
...
static int _flash_write(size_t addr, size_t size, uint8_t *buf)
{
	...
	do {
		...
		hal_flash_read(start_addr, backup, FLASH_SECTOR_SIZE);
		pos = addr - start_addr;
