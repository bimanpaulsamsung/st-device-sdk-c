...
static char heap_info_buf[BUF_SIZE] = {0,};
...
void iot_bsp_debug_check_heap(const char* tag, const char* func, const int line, const char* fmt, ...)
{
	...
	va_start(va, fmt);
	vsnprintf(heap_info_buf, BUF_SIZE, fmt, va);
	va_end(va);
	...
	iot_bsp_debug(IOT_DEBUG_LEVEL_WARN, tag, "%s(%d) > " "[MEMCHK][%d][%s] "
			"Current Size:%d, Maximum Size:%d, Total Size:%d, Reserved Size:%d, Overhead Size:%d", func, line, ++count, heap_info_buf,
