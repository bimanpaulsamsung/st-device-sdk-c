...
/**
 * @brief Connection parameterd for persistent session
 */
typedef struct  iot_conn_params {
	unsigned int tcp_idle; /**< @brief tcp keep alive idle with seconds unit */
	unsigned int tcp_interval; /**< @brief tcp keep alive intrval */
	unsigned int tcp_count; /**< @brief tcp keep alive count */
} iot_conn_params_t;
 ...
 * @brief Contains a enumeration values for types of notification.
 */
typedef enum enum_keepalive {
	IOT_KEEPALIVE_FAIL = -1,		/**< @brief For Error. */

	IOT_KEEPALIVE_SUCCESS,		/**< @brief For Success. */
	IOT_KEEPALIVE_VALUE		/**< @brief For Keepalive value. */
} enum_keepalive_t;
...
typedef void (*st_keepalive_cb)(enum_keepalive_t, iot_conn_params_t *conn);
/**
...
* @details	This function set connection parameters for persistent session
* @param[in]	iot_ctx	iot_context handle generated by st_conn_init()
* @param[in]	conn	 connection parameters
* @return 		return `(0)` if it works successfully, non-zero for error case.
*/
